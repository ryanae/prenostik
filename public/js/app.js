...
(function($) {
  ...
  var AppRouter = Backbone.Router.extend({
    ...
    home: function(){
      this.view = ReactDOM.render(<HomeComponent />, 
    ...
    trending: function(snapshotId) {
      ...
      this.view = ReactDOM.render(<AnalyzeComponent 
    ...
    snapshot: function(categoryId) {
      ...
      if (analyzeView.length > 0 
          && this.view 
          && typeof this.view.prepareRemove === 'function'){
  ...
  $('a[href="/users/logout"]').off().on('click', function(e) {
    ...
    var support = new accountsCommon();
    var currentUser = support.getAuthUser();
    var cachedSnapshots = store.get('snapshots:'+currentUser.id);
    if(cachedSnapshots){
